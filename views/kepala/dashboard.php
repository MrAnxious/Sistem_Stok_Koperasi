...
// Ambil Total Barang
$query_total_barang = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM stok");
$total_barang = mysqli_fetch_assoc($query_total_barang)['total'] ?? 0;

// Ambil Barang Masuk Hari Ini
$tgl_hari_ini = date('Y-m-d');
$query_masuk_hari_ini = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM barang_masuk WHERE DATE(tanggal_masuk) = '$tgl_hari_ini'");
$masuk_hari_ini = mysqli_fetch_assoc($query_masuk_hari_ini)['total'] ?? 0;

// Ambil Barang Keluar Hari Ini
$query_keluar_hari_ini = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM barang_keluar WHERE DATE(tanggal_keluar) = '$tgl_hari_ini'");
$keluar_hari_ini = mysqli_fetch_assoc($query_keluar_hari_ini)['total'] ?? 0;

// Ambil Jumlah Supplier
$query_supplier = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM supplier");
$total_supplier = mysqli_fetch_assoc($query_supplier)['total'] ?? 0;

// Ambil Stok Menipis (Kurang dari 10)
$query_stok_menipis = mysqli_query($koneksi, "SELECT * FROM stok WHERE jumlah_stok < 10 ORDER BY jumlah_stok ASC LIMIT 5");

// Ambil Transaksi Terakhir (Gabungan Masuk & Keluar) menggunakan UNION ALL
...
";
$result_trx_terakhir = mysqli_query($koneksi, $query_trx_terakhir);
?>
<!DOCTYPE html>
...
<head>
    ...
    <?php endif; ?>
    <title><?= htmlspecialchars($_SETTINGS['nama_sistem'] ?? 'Koperasi Merah Putih') ?> - Dashboard Kepala</title>
...
<body class="text-gray-800 min-h-screen flex" x-data="{ isSidebarOpen: false }">
    ...
    <main class="flex-1 flex flex-col min-w-0 w-full overflow-hidden">
        ...
        <div class="flex-1 px-4 sm:px-6 lg:px-12 py-8 overflow-y-auto w-full">
            ...
            <div class="mb-8">
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-gray-900">Ringkasan Operasional (Mode Pantau)</h1>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8 lg:mb-12">
                ...
                <div class="bg-gradient-to-br from-primary to-red-800 p-5 sm:p-6 rounded-2xl text-white flex flex-col justify-between h-36 sm:h-40 lg:h-48 shadow-lg shadow-red-900/20">
                    ...
                    <div>
                        <p class="text-white/80 text-xs sm:text-sm font-medium">Total Jenis Barang</p>
                ...
                <div class="bg-white p-5 sm:p-6 rounded-2xl border border-gray-100 flex flex-col justify-between h-36 sm:h-40 lg:h-48 shadow-sm">
                    ...
                    <div>
                        <p class="text-gray-500 text-xs sm:text-sm font-medium">Transaksi Barang Masuk</p>
                ...
                <div class="bg-white p-5 sm:p-6 rounded-2xl border border-gray-100 flex flex-col justify-between h-36 sm:h-40 lg:h-48 shadow-sm">
                    ...
                    <div>
                        <p class="text-gray-500 text-xs sm:text-sm font-medium">Transaksi Barang Keluar</p>
                ...
                <div class="bg-gray-900 p-5 sm:p-6 rounded-2xl flex flex-col justify-between h-36 sm:h-40 lg:h-48 shadow-lg shadow-gray-900/20">
                    ...
                    <div>
                        <p class="text-gray-400 text-xs sm:text-sm font-medium">Jumlah Mitra Supplier</p>
            ...
            <div class="flex flex-col lg:flex-row gap-6 lg:gap-8 w-full">
                ...
                <div class="w-full lg:w-2/3 bg-white p-5 sm:p-8 rounded-2xl border border-gray-100 shadow-sm">
                    <div class="mb-6">
                        ...
                        <p class="text-gray-500 text-xs sm:text-sm mt-1">Perbandingan tren transaksi barang masuk dan keluar.</p>
                    </div>
                    
                    <div class="relative w-full text-center flex items-center justify-center flex-col bg-white rounded-xl">
                ...
                <div class="w-full lg:w-1/3 flex flex-col gap-6">
                    ...
                    <div class="bg-white p-5 sm:p-6 rounded-2xl border border-gray-100 shadow-sm flex-1">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="font-bold text-gray-800">Peringatan: Stok Menipis</h3>
                        </div>
                        <div class="space-y-4">
                            <?php if (mysqli_num_rows($query_stok_menipis) > 0): ?>
                                <?php while($stok_habis = mysqli_fetch_assoc($query_stok_menipis)): ?>
                                <div class="flex items-center gap-3 sm:gap-4 p-3 bg-red-50/50 rounded-xl border border-red-100">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-bold text-gray-800 truncate"><?= htmlspecialchars($stok_habis['nama_barang']); ?></p>
                                        <p class="text-xs text-red-600 font-medium">Sisa: <?= $stok_habis['jumlah_stok']; ?> unit</p>
                                    </div>
                                </div>
                                <?php endwhile; ?>
                            <?php else: ?>
                        ...
                        <div class="mt-6">
                            <a href="laporan_stok.php" class="text-sm text-primary font-medium hover:underline flex items-center gap-1 justify-center">
                                Lihat Semua Stok <span class="material-symbols-outlined text-sm">arrow_forward</span>
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-8 lg:mt-12 bg-white rounded-2xl p-5 sm:p-8 border border-gray-100 shadow-sm w-full overflow-hidden">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <h2 class="text-lg sm:text-xl font-bold text-gray-800">Riwayat Transaksi Terakhir</h2>
                    <div class="flex items-center gap-4">
                        ...
                        <a href="riwayat_transaksi.php" class="text-sm text-primary font-medium hover:underline flex items-center gap-1">
                            Lihat Lebih Banyak <span class="material-symbols-outlined text-sm">arrow_forward</span>
                        </a>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left min-w-[700px]">
                        <thead>
                            <tr class="text-xs font-bold text-gray-500 border-b border-gray-200 uppercase tracking-wider bg-gray-50">
                                ...
                                <th class="p-4 text-right rounded-tr-lg">Jumlah</th>
                            </tr>
                        </thead>
                        <tbody class="text-sm divide-y divide-gray-100">
                            <?php if (mysqli_num_rows($result_trx_terakhir) > 0): ?>
                                <?php while($trx = mysqli_fetch_assoc($result_trx_terakhir)): 
                                    $prefix = ($trx['tipe'] == 'MASUK') ? 'TM' : 'TK';
                                    $no_trx = $prefix . str_pad($trx['id_trx'], 3, '0', STR_PAD_LEFT);
                                ?>
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="p-4 font-semibold text-gray-800"><?= $no_trx ?></td>
                                    <td class="p-4 text-gray-500 font-medium"><?= date('d M Y, H:i', strtotime($trx['waktu'])); ?></td>
                                    <td class="p-4 font-semibold text-gray-800"><?= htmlspecialchars($trx['nama_barang']); ?></td>
                                    <td class="p-4"><span class="px-3 py-1 bg-gray-100 rounded-md text-xs font-medium text-gray-600"><?= htmlspecialchars($trx['kategori']); ?></span></td>
                                    <td class="p-4">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" class="p-8 text-center text-gray-500">Belum ada transaksi sama sekali.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
