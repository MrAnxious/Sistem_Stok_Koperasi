...
<head>
    ...
    <?php endif; ?>
    <title>Profil Saya - <?= htmlspecialchars($_SETTINGS['nama_sistem'] ?? 'Koperasi Merah Putih') ?></title>
...
<body class="text-gray-800 min-h-screen flex" x-data="{ isSidebarOpen: false }">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-72 bg-white flex flex-col h-screen transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto shadow-xl lg:shadow-none border-r border-gray-200" :class="{'translate-x-0': isSidebarOpen, '-translate-x-full': !isSidebarOpen}">
        ...
        <div class="p-4 border-t border-gray-200 space-y-2">
            <a href="profil.php" class="w-full flex items-center gap-3 px-4 py-3 bg-primary/10 text-primary font-semibold rounded-xl transition-colors relative">
                <div class="absolute left-0 w-1 h-6 bg-primary rounded-r-full"></div>
                <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">person</span><span>Profil Saya</span>
            </a>
        ...
        <div class="flex-1 px-4 sm:px-6 lg:px-12 py-8 overflow-y-auto w-full flex justify-center">
            ...
            <div class="w-full max-w-2xl bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden self-start">
                <div class="p-8 border-b border-gray-50 flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="font-bold text-2xl text-gray-900">Pengaturan Profil</h2>
                        <p class="text-gray-500 mt-1">Perbarui username atau kata sandi Anda di sini.</p>
                    </div>
                </div>

                <form action="../../proses/update_profil.php" method="POST" enctype="multipart/form-data" class="p-8">
                    <input type="hidden" name="id_user" value="<?= $user['id_user'] ?>">
                    <input type="hidden" name="foto_lama" value="<?= htmlspecialchars($user['foto'] ?? '') ?>">
                    
                    <div class="mb-8 flex items-center gap-6">
                        <?php if(!empty($user['foto'])): ?>
                            <img alt="Profil" class="w-24 h-24 rounded-full object-cover border-4 border-gray-100 shadow-sm" src="../../assets/kepala/<?= htmlspecialchars($user['foto']); ?>"/>
                        <?php else: ?>
                            <div class="w-24 h-24 rounded-full bg-primary/20 flex items-center justify-center border-4 border-gray-100 text-primary shadow-sm">
                                <span class="material-symbols-outlined text-5xl">person</span>
                            </div>
                        <?php endif; ?>
                        <div>
                            <h2 class="text-xl font-bold text-gray-900"><?= htmlspecialchars($user['username']) ?></h2>
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 mt-2 rounded-lg bg-gray-100 text-xs font-semibold text-gray-600 uppercase tracking-wide">
                                <span class="material-symbols-outlined text-[14px]">shield_person</span> <?= htmlspecialchars($user['role']) ?>
                            </span>
                        </div>
                    </div>

                    <?php if (isset($_SESSION['pesan_profil'])): ?>
                        <div class="mb-6 p-4 rounded-xl <?= strpos($_SESSION['pesan_profil'], 'Gagal') !== false ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200' ?> flex items-center gap-3">
                            <span class="material-symbols-outlined"><?= strpos($_SESSION['pesan_profil'], 'Gagal') !== false ? 'error' : 'check_circle' ?></span>
                            <span class="text-sm font-medium">
                                <?php 
                                    echo $_SESSION['pesan_profil']; 
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            <label class="text-sm font-semibold text-gray-700 flex items-center gap-2">
                                <span class="material-symbols-outlined text-sm">person</span> Username Baru
                            </label>
                            <input type="text" name="username" value="<?= htmlspecialchars($user['username']) ?>" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all">
                        </div>
                        
                        <div class="space-y-2 pt-2 border-t border-gray-100">
                            <label class="block text-sm font-semibold text-gray-700 mb-1 flex items-center gap-2">
                                <span class="material-symbols-outlined text-sm">key</span> Password Baru (Opsional)
                            </label>
                            <input type="password" name="password" placeholder="Kosongkan jika tidak ingin mengubah password" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all">
                        </div>
                        
                        <div class="space-y-2">
                            <label class="block text-sm font-semibold text-gray-700 mb-1 flex items-center gap-2">
                                <span class="material-symbols-outlined text-sm">password</span> Konfirmasi Password Baru
                            </label>
                            <input type="password" name="k_password" placeholder="Ulangi password baru" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all">
                        </div>
                        
                        <div class="space-y-2 pt-2 border-t border-gray-100">
                            <label class="block text-sm font-semibold text-gray-700 mb-1 flex items-center gap-2">
                                <span class="material-symbols-outlined text-sm">image</span> Ganti Foto Profil (Opsional)
                            </label>
                            <input type="file" name="foto" accept="image/*" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white hover:file:bg-red-800 cursor-pointer">
                        </div>
                    </div>
